  ...
  return (
    <StoryCard gradient="from-cyan-500 via-teal-500 to-emerald-600">
      <div className="text-center space-y-8 max-w-2xl w-full px-4">
        ...
        >
          <Hourglass size={72} className="mx-auto text-white/90" />
        </motion.div>
        ...
        >
          <h2 className="text-4xl font-bold mb-2">Мастер тайм-менеджмента</h2>
          <p className="text-xl opacity-90">Ты точно знаешь, сколько нужно быть на паре</p>
        </motion.div>
        ...
        <motion.div
          ...
          className="space-y-6"
        >
          <div className="bg-white/20 backdrop-blur-md rounded-3xl p-8 inline-block w-full max-w-md">
            <p className="text-lg opacity-80 mb-2">Чаще всего ты «оптимизировал»</p>
            <p className="text-3xl font-bold mb-6 leading-tight">{partialStats.name}</p>
            
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/10 rounded-2xl p-4">
                <div className="text-4xl font-bold">{partialStats.count}</div>
                <div className="text-sm opacity-80">раз(а)</div>
              </div>
              <div className="bg-white/10 rounded-2xl p-4">
                <div className="text-4xl font-bold">{partialStats.avgPercent}%</div>
        ...
        >
          <Timer size={24} />
          <p className="text-xl">В среднем по {partialStats.avgMinutes} мин.</p>
        </motion.div>
