...
export function Story15Final() {
  const { studentData, semester, setCurrentStory } = useAppStore();
  ...
  const stats = useMemo(() => {
    ...
    // Total time
    const totalSeconds = studentData.scud.days.reduce(
      (acc, day) => acc + day.total_time_seconds,
      0,
    );
    ...
    const subjectGroups = new Map<
      string,
      { count: number; subgroups: Set<string> }
    >();
    ...
    return [
      ...
      {
        value: avgGrade > 0 ? avgGrade.toFixed(2) : "-",
        label: "средний балл",
      },
    ];
  }, [studentData]);
  ...
  return (
    <StoryCard gradient="from-indigo-700 via-blue-700 to-cyan-600">
      <ParticleBackground count={25} emoji={emojis} speed={8} />
      ...
      <div className="z-10 text-center space-y-4 md:space-y-8 max-w-2xl">
        ...
        >
          <h1 className="text-3xl md:text-5xl mb-2 md:mb-4">
            Это был твой семестр
          </h1>
        ...
        >
          {semester === 'spring' ? 'Отличного отдыха! ☀️✨' : 'С Новым годом! 🎄✨'}
        </motion.div>
      </div>
    </StoryCard>
  );
